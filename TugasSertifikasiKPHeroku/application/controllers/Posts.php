...
class Posts extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $config['base_url'] = "https://mybloggu.herokuapp.com/posts/index";
    ...
    {
        if ($this->session->userdata('keywordStatus') == false) {
            $this->session->set_userdata('keywordStatus', '');
        }

        if (isset($_POST['submit'])) {
            $data['keywordStatus'] = $this->input->post('keywordStatus');
            $this->session->set_userdata('keywordStatus', $data['keywordStatus']);
        } else {
            $data['keywordStatus'] = $_SESSION['keywordStatus'];
        }

        #load library pagination
        $this->load->library('pagination');

        $config['total_rows'] = $this->Post_model->banyakPostStatus($data['keywordStatus']);
        $config['base_url'] = "https://mybloggu.herokuapp.com/posts/index";
        #$config['total_rows'] = $this->Post_model->banyakSemuaPost();
        $config['per_page'] = 5;
        ...
        $data['start'] = $this->uri->segment(3);
        $data['posts'] = $this->Post_model->getPencarianStatus($config['per_page'], $data['start'], $data['keywordStatus']);
